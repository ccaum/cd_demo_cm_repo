...
class puppet::master (
  ...
  $puppet_master_package_source   = undef,
  $puppet_master_package_provider = undef,
  ...
  if ! defined(Package[$puppet_master_package]) {
    package { $puppet_master_package:
      ...
      source   => $puppet_master_package_source,
      provider => $puppet_master_package_provider,
    }
  }
  ...
  if $config_version != 'UNSET' {
    ini_setting { 'puppetmasterconfigversion':
      ensure => present,
  ...
  file { 'puppetdb.conf':
    path    => "${::puppet::params::confdir}/puppetdb.conf",
