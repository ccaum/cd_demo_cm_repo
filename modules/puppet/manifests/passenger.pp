...
class puppet::passenger(
  ...
  if $::osfamily == 'redhat' {
    package { 'libev-4.04-1.el6.x86_64':
      source   => 'http://dl.marmotte.net/rpms/redhat/el6/x86_64/libev-4.04-1.el6/libev-4.04-1.el6.x86_64.rpm',
      before   => Package['passenger-release-3-6.el6.noarch'],
      provider => rpm,
    }
      
    package { 'passenger-release-3-6.el6.noarch':
      ensure   => installed,
      source   => 'http://passenger.stealthymonkeys.com/rhel/6/passenger-release.noarch.rpm',
      before   => Package[$::puppet::params::passenger_package],
      provider => rpm,
    }
